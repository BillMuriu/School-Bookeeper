...
import {
  useEditTuitionOpeningBalance,
  useDeleteTuitionOpeningBalance,
} from "../_services/mutations"; // Adjusted imports for tuition mutation hooks
...
const EditDeleteOpeningBalanceForm = ({ openingBalanceId }) => {
  ...
  const editOpeningBalanceMutation = useEditTuitionOpeningBalance(); // Hook for editing tuition opening balance
  const deleteOpeningBalanceMutation = useDeleteTuitionOpeningBalance(); // Hook for deleting tuition opening balance
  ...
  const onDelete = () => {
    ...
    deleteOpeningBalanceMutation.mutate(openingBalanceId, {
      onSettled: () => {
        setIsLoading(false);
        router.push("/tuition-opening-balance"); // Adjusted redirect path for tuition account
      },
    });
  };
