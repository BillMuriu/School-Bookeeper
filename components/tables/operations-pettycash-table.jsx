...
export function PettyCashTable({ columns, data, onSelectionChange }) {
  ...
  return (
    <div>
      <div className="flex items-center py-4">
        <Input
          placeholder="Filter By Cheque No..."
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  const isLeftAligned =
                    header.column.columnDef.header === "Cheque No";
                  return (
                    <TableHead
                      key={header.id}
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => {
                    const isLeftAligned =
                      cell.column.columnDef.header === "Cheque No";
                    return (
                      <TableCell
                        key={cell.id}
                        className={isLeftAligned ? "text-left" : "text-center"}
